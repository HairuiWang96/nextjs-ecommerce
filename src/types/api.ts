...
export function createErrorResponse(message: string, code: ErrorCode, details?: Record<string, string[]>): Extract<ApiResponse<never>, { success: false }> {
    return {
        success: false,
...
export function createSuccessResponse<T>(data: T): Extract<ApiResponse<T>, { success: true }> {
    return {
        success: true,
        data,
    };
