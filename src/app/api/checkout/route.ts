...
import { cartStore, orderStore, inventoryManager, idempotencyStore, generateId } from '@/lib/store';
    ...
    try {
        ...
        if (!reservationResult.success) {
            return NextResponse.json(
                createErrorResponse(`Out of stock: ${reservationResult.failedItem}`, ErrorCode.OUT_OF_STOCK),
                { status: 409 }, // 409 Conflict — resource state conflict
            );
        }

        // ---- Step 4: Process payment (with retry) ----
        const paymentResult = await processPaymentWithRetry(cart.totals.total, body.payment.token);
