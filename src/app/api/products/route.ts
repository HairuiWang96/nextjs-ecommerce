...
import { createSuccessResponse, createErrorResponse, paginate, ErrorCode, type Product, type CreateProductInput, type PaginatedResponse, type ApiResponse } from '@/types';

// PATTERN: Typed GET handler with query parameters
export async function GET(request: NextRequest): Promise<NextResponse<ApiResponse<PaginatedResponse<Product>>>> {
    ...
    if (search) {
        ...
        products = products.filter(p => p.title.toLowerCase().includes(lowerSearch) || p.tags.some(t => t.includes(lowerSearch)));
    }
...
export async function POST(request: NextRequest): Promise<NextResponse<ApiResponse<Product>>> {
    try {
        ...
        if (Object.keys(errors).length > 0) {
            return NextResponse.json(createErrorResponse('Validation failed', ErrorCode.VALIDATION_ERROR, errors), { status: 400 });
        }
        ...
        return NextResponse.json(createErrorResponse('Invalid request body', ErrorCode.BAD_REQUEST), { status: 400 });
    }
}
